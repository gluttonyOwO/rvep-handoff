...
操作員使用 **網頁瀏覽器**（Chrome/Edge/Firefox）連線至 RVEP Web Cockpit，流程如下：

1. **登入** — 輸入帳號密碼，取得 JWT Access Token
2. **選擇車輛** — 從儀表板列表選取有權限的車輛
3. **進入駕駛艙** — 自動建立 Session、取得 ControlLease、連接 LiveKit Room
4. **即時操控** — 透過滑鼠/觸控虛擬搖桿或實體 Gamepad（Xbox/PS5）發送 Movement Command
5. **雙路影像** — 接收車輛前/後 ZED-X 相機 H.264 即時串流
6. **監控狀態** — Telemetry HUD 顯示速度、電量、GPS、IMU、Network RTT
7. **安全機制** — 心跳異常時自動進入 Safe Mode，操作員需手動確認恢復
8. **離開** — 釋放 ControlLease、關閉 Session、斷開 LiveKit 連線

### 1.2 使用終端

| 終端 | 用途 | 必備條件 |
|------|------|----------|
| **桌機（操作員）** | 開啟瀏覽器操作 Web Cockpit，使用滑鼠搖桿或外接 Gamepad | Chrome/Edge/Firefox，建議有 Gamepad（Xbox/PS5） |
| **筆電（操作員）** | 開啟瀏覽器操作 Web Cockpit，使用觸控板/滑鼠搖桿或外接 Gamepad | Chrome/Edge/Firefox，建議有 Gamepad（Xbox/PS5） |
| **平板（操作員）** | 觸控螢幕操作虛擬搖桿，適合現場巡檢攜帶 | iPad/Android 平板，Chrome/Safari |
| **手機（操作員/觀察者）** | 瀏覽器連線觀看車輛狀態與影像，可進行基本控制 | iPhone/Android，Chrome/Safari |
| **Jetson（邊緣裝置）** | 安裝於車輛端，運行 Go Video Publisher（NVENC 硬體編碼）與 Edge Agent，負責影片串流與 IPC 管理 | Ubuntu 24.04、GStreamer、ZED-X 相機 |
| **Jetson / 嵌入式電腦（R2 車輛）** | 安裝於車輛端，運行 R2 Bridge（Python ROS2），負責 DataChannel ↔ ROS2 橋接 | Ubuntu 24.04、ROS2、Python 3.10+ |
| **伺服器（後端 + LiveKit SFU）** | 部署在機房或雲端，運行 Next.js Backend API + LiveKit SFU + PostgreSQL | Linux 伺服器、Docker、Node.js 18+ |

### 1.3 操作目的

| 目標 | 說明 |
|------|------|
| **遠程車輛監控** | 操作員可在地圖上查看車輛位置、即時 Telemetry（速度/電量/IMU）、雙路 H.264 影像 |
| **遠程車輛控制** | 透過 Gamepad 或觸控搖桿即時控制車輛移動（前進/後退/旋轉），支援緊急停止 |
| **安全駕駛** | 心跳超時自動安全模式、操作員需手動確認恢復，防止通訊中斷時車輛失控 |
| **AI 資料收集** | 車輛運行時自動錄製影像 + 感測器資料 + JSONL 元數據，用於後續 AI 訓練 |
| **車隊管理** | Admin 可管理多台車輛與操作員權限，Operator 僅能操作被授權的車輛 |

### 1.4 所需設備

| 類別 | 設備 | 數量 | 用途 |
|------|------|------|------|
| **車輛** | Wheeltec AMR R2（差速驅動）或同級 ROS2 UGV | 1+ | 被控車輛底盤 |
| **相機** | ZED-X 立體相機（GMSL2 介面） | 2（前/後） | 雙路即時影像 |
| **邊緣運算** | NVIDIA Jetson（64GB） | 1 | Go Video Publisher + Edge Agent |
| **車載運算** | Jetson 或同等嵌入式電腦 | 1 | R2 Bridge（ROS2 橋接） |
| **伺服器** | Linux 主機（實體/雲端） | 1 | Backend API + LiveKit SFU + PostgreSQL |
| **操作終端** | PC / 筆電（Windows/macOS/Linux） | 1+ | Web Cockpit 瀏覽器操作 |
| **控制器** | Xbox / PS5 Gamepad（選配） | 1+ | 實體搖桿控制車輛 |
| **網路** | 4G/5G 路由器或區域網路 | 1 | 車輛 ↔ 伺服器通訊 |

### 1.5 架設位置

| 元件 | 部署位置 | 說明 |
|------|----------|------|
| **Backend API** | 機房伺服器或雲端 VM | 需固定 IP/域名供所有端點連線 |
| **LiveKit SFU** | 同 Backend 或獨立伺服器 | 低延遲需求，建議與 Backend 同區域網路 |
| **PostgreSQL** | 同 Backend 或獨立資料庫 | 可 docker-compose 一併部署 |
| **Web Cockpit** | 瀏覽器端（無需安裝） | 透過 HTTPS 連線至 Backend |
| **Edge Agent + Go Publisher** | Jetson（車輛端） | 安裝於車輛上，透過 4G/5G 或 Wi-Fi 連至 LiveKit |
| **R2 Bridge** | Jetson（車輛端） | 與 ROS2 Master 同機或同網段，經 LiveKit DataChannel 通訊 |

### 1.6 架設方法

#### 後端 + LiveKit + 資料庫（伺服器端）

使用 docker-compose 一次性部署：

```bash
# 克隆專案
git clone https://github.com/gluttonyOwO/rvep-handoff.git
cd rvep-handoff

# 啟動後端服務（Backend API + LiveKit + PostgreSQL）
docker compose up -d
```
...
# 安裝依賴
sudo apt update
sudo apt install -y golang-go libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev \
  libgstreamer-plugins-bad1.0-dev gstreamer1.0-plugins-bad gstreamer1.0-tools pkg-config

# 建置 Go Publisher
cd apps/edge-publisher-go
go build -o rvep-publisher ./cmd/publisher

# 設定相機 YAML Profile（/etc/rvep/cameras/zed-x-front.yaml）
# 設定環境變數後啟動 mock-edge
cd apps/mock-edge
cp .env.example .env   # 編輯 LIVEKIT_URL、VEHICLE_ID 等
pnpm start
```

#### R2 Bridge（Jetson / 車輛端）

```bash
# 安裝 ROS2 + Python 依賴
pip install "livekit-api>=0.8.1" "livekit-protocol>=0.8.1"

# 設定環境變數後啟動
cd apps/r2-bridge
export VEHICLE_ID="r2-001"
export LIVEKIT_URL="ws://your-server:7880"
export ROOM="ugv-vehicle-001"
python -m r2_bridge.main
```
...
上圖展示 RVEP 系統中所有元件之間的通訊協定與資料流方向：

- **Web 前端（Port 3011）** — Operator Cockpit 透過 HTTPS REST 與後端通訊（登入/車輛列表/Lease），並以 WebRTC 連接 LiveKit SFU 接收 H.264 影片與 DataChannel
- **後端 API（Port 3010）** — Next.js App Router 提供 REST 端點，JWT Middleware 驗證身分，Prisma ORM 操作 PostgreSQL
- **LiveKit SFU（Port 7880）** — WebRTC SFU 負責影片轉發 + 雙向 DataChannel（ControlCommand → Edge、Telemetry → Cockpit、SafetyEvent、Heartbeat 1Hz）
- **邊緣裝置（Jetson）** — Go Publisher 透過 GStreamer NVENC 硬體編碼 H.264，經 Unix Socket IPC 由 Edge Agent 管理生命週期；Edge Agent 另以 5Hz 發布 Telemetry 至 DataChannel
- **車輛層** — R2 Bridge（Python）訂閱 DataChannel 接收控制命令，轉譯為 ROS2 `/rvep/cmd_vel` 驅動車輛；車輛回傳 `/odom`、`/IMU`、`/Battery` 等 ROS2 Topics，由 R2 Bridge 編碼為 Telemetry 送回 Cockpit
- **開發支援** — Mock Edge Agent 模擬邊緣裝置行為（假資料），`@rvep/shared` Zod Schemas 在模組間共享型別定義
...
操作員輸入 Email/Password 登入，後端驗證 bcrypt hash 後回傳 Access Token + Refresh Cookie。前端攜帶 Bearer Token 查詢 `/api/v1/vehicles`，後端依 VehiclePermission 過濾回傳有權限的車輛清單。

### 6.2 時序圖 2/7 — 獲取控制租約

![sequence-02-claim-lease](sequence-02-claim-lease.svg)

操作員選擇車輛後，建立 Session 並透過 `pg_advisory_xact_lock` 確保 ControlLease 不重入（30min TTL），最後向 LiveKit 請求 operator role 的 Room Token（含 roomJoin + canPublish + canSubscribe 權限）。

### 6.3 時序圖 3/7 — 連線 LiveKit 與串流

![sequence-03-livekit-stream](sequence-03-livekit-stream.svg)

Cockpit 加入 LiveKit Room 訂閱 Video/DataChannel。Edge Agent 透過 IPC 啟動 Go Publisher（GStreamer NVENC H.264 編碼），Go Publisher 以獨立 participant 身分發布 Video Track。每秒 IPC heartbeat 回報編碼狀態（FPS、dropped frames）。

### 6.4 時序圖 4/7 — 控制命令與 Telemetry

![sequence-04-control-telemetry](sequence-04-control-telemetry.svg)

操作員拖曳 Joystick → Cockpit 以 10Hz throttle 編碼 MovementCommand → LiveKit DataChannel (unreliable) → R2 Bridge 解碼為 ROS2 `/rvep/cmd_vel`。車輛回傳 odom/IMU/battery → R2 Bridge 以 5Hz 編碼 Telemetry 送回 Cockpit 更新 HUD。雙路 Telemetry：R2 Bridge（真實感測器）+ Edge Agent（Mock 資料，開發用）。

### 6.5 時序圖 5/7 — 心跳安全機制

![sequence-05-heartbeat](sequence-05-heartbeat.svg)

Cockpit 每秒發送 heartbeat（reliable=false）經 LiveKit DataChannel 至 Edge Agent 與 R2 Bridge。Edge Agent 每 500ms 檢查心跳新鮮度，R2 Bridge watchdog 每 500ms 檢查 3 秒超時。任一方超時即觸發安全模式：Edge Agent 停止 Go Publisher，R2 Bridge 發布 ROS2 零速度指令。

### 6.6 時序圖 6/7 — 異常場景：心跳超時與恢復

![sequence-06-safe-mode-recovery](sequence-06-safe-mode-recovery.svg)

心跳中斷 > 3 秒 → Edge Agent 進入 safe_mode（停止 video）、R2 Bridge 發布 ROS2 `/rvep/emergency_stop`。Cockpit 顯示 SafetyBanner + RecoveryModal（全屏 focus trap）。操作員確認安全後點擊「恢復控制權」，Edge Agent 確認心跳新鮮度 < 1s 後 leaveSafeMode()，重啟 Go Publisher 恢復串流。

### 6.7 時序圖 7/7 — 釋放控制

![sequence-07-release](sequence-07-release.svg)

操作員離開駕駛艙 → Cockpit 釋放 ControlLease（狀態 RELEASED + releasedAt）→ 離開 LiveKit Room → Edge Agent 收到 ParticipantDisconnect Event → 停止 Go Publisher video → 進入 safe_mode（operator_disconnected）→ 關閉 Session（狀態 CLOSED），完整生命週期結束。
...
### 7.11 JWT Token 說明：前端 Auth JWT vs LiveKit SFU JWT

系統中有兩種不同的 JWT Token，用途與生命週期完全不同：

| 面向 | Auth JWT（後端 API） | LiveKit JWT（SFU） |
|------|---------------------|-------------------|
| **用途** | 驗證 HTTP REST API 請求（`Authorization: Bearer`） | 授權加入 LiveKit Room 進行 WebRTC 連線 |
| **發行者** | 後端 Next.js（`apps/backend/src/lib/auth.ts`） | 後端 Next.js（`apps/backend/src/lib/livekit.ts`） |
| **簽發端點** | `POST /api/v1/auth/login` | `POST /api/v1/livekit/token` |
| **簽發對象** | Operator / Admin / Viewer（使用者） | Operator / Viewer / Edge Agent / Go Publisher |
| **承載內容** | `{ userId, role, refreshTokenVersion }` | `{ identity, room, grant: { roomJoin, canPublish, canSubscribe, canPublishData } }` |
| **TTL** | Access Token: **1 小時** | **1 小時**（`TOKEN_TTL_SECONDS = 3600`） |
| **更新方式** | 自動 silent refresh（到期前 5 分鐘）+ 401 自動重試 | **無自動更新機制**（參見 §9.9） |
| **金鑰** | `JWT_SIGNING_KEY` / `JWT_REFRESH_KEY`（Base64 HS256） | `LIVEKIT_API_KEY` + `LIVEKIT_API_SECRET` |
| **撤銷方式** | `refreshTokenVersion` 遞增（DB 欄位） | 不可撤銷（到期自然失效） |
| **傳輸方式** | HTTP Header `Authorization: Bearer <token>` | WebRTC SDP 協商時傳遞 |

#### Auth JWT 流程（前端 → 後端 API）

```
登入 → 取得 Access Token（存入記憶體） + Refresh Cookie（httpOnly, SameSite=strict）
       │
       ▼
   後續 API 請求：Header Authorization: Bearer <accessToken>
       │
       ├─ 成功 → 正常回應
       │
       └─ 401 Unauthorized
               │
               ├─ 自動呼叫 POST /auth/refresh（附帶 Refresh Cookie）
               │       └─ 成功 → 新 Access Token + 重試原始請求
               │       └─ 失敗 → 強制登出
               │
               └─ 定時器：Access Token 到期前 5 分鐘自動 refresh
```

- 實作位於 `apps/web/src/lib/api-client.ts:36-50`（`REFRESH_MARGIN_MS = 5 * 60 * 1000`）
- Refresh Token 本身也有 **14 天 TTL**，每次 refresh 會遞增 `refreshTokenVersion` 防止 replay

#### LiveKit JWT 流程（前端 → LiveKit SFU）

```
操作員進入駕駛艙
       │
       ▼
POST /api/v1/livekit/token { vehicleId, role: "operator" }
       │
       ▼
後端驗證：
  1. 確認使用者已登入（Auth JWT Middleware）
  2. 確認車輛存在
  3. 確認使用者有 VehiclePermission
  4. 確認 ControlLease 有效（operator role）
  5. 簽發 LiveKit JWT：
     - identity: "{role}-{userId}"（如 operator-clj4abc...）
     - room: "ugv-{vehicleId}"
     - grant: roomJoin + canPublish + canPublishData + canSubscribe
  6. TTL: 3600 秒
       │
       ▼
前端接收 { token, url, roomName, identity, expiresAt }
       │
       ▼
room.connect(url, token)  → 加入 WebRTC Room
```

- Edge Agent 與 Go Publisher 使用不同的 LiveKit JWT（identity 規則參見 §10.3）
- Go Publisher 的 token 由 Edge Agent 透過 `livekit-publisher-token.ts` 簽發（僅 `canPublish`，無 subscribe 權限）

### 7.12 Backend 與 LiveKit SFU 通訊

Backend 與 LiveKit SFU 之間的通訊極簡，僅限 **Token 簽發**，無任何即時或 REST 管理呼叫：

| 通訊方式 | 是否存在 | 說明 |
|---------|---------|------|
| **Token 簽發** | ✅ | 使用 `livekit-server-sdk` 的 `AccessToken` + `VideoGrant` 離線簽發 JWT（不需連線至 SFU） |
| **REST API 管理**（createRoom / listRooms / DeleteRoom / SendData） | ❌ | 未使用 `RoomServiceClient`，Backend 不管理 Room 生命週期 |
| **Webhook 接收**（ParticipantDisconnect / RoomEnded 等事件） | ❌ | 無 webhook endpoint，Backend 不接收 LiveKit 事件 |
| **Participant 查詢** | ❌ | 未查詢 SFU 中的參與者狀態 |
| **直接 WebRTC 連線** | ❌ | Backend 不加入 Room，不發布/訂閱任何 Track 或 DataChannel |

**Backend 目前的 LiveKit 整合僅一條路徑：**

```
客戶端（Cockpit / Edge Agent）→ POST /api/v1/livekit/token → Backend
                                                              │
                                                              ├─ 驗證使用者身分（Auth JWT）
                                                              ├─ 驗證 VehiclePermission
                                                              ├─ 驗證 ControlLease（operator）
                                                              └─ livekit-server-sdk AccessToken
                                                                   └─ 離線簽發 LiveKit JWT（1hr TTL）
                                                                       └─ 回傳給客戶端
                                                                           └─ 客戶端自行用此 Token 連線至 LiveKit SFU
```
...
| 套件 | 版本 | 位置 | 用途 |
|------|------|------|------|
| `livekit-server-sdk` | ^2.7.0 | `apps/backend/` | Token 簽發 + 測試驗證 |
| `livekit-client` | ^2.7.0 | `apps/web/` | 瀏覽器端 WebRTC + DataChannel |
| `@livekit/rtc-node` | ^0.13.0 | `apps/mock-edge/` | Node.js 端 Room Participant |
| `livekit-server-sdk` | ^2.8.0 | `apps/mock-edge/` | Publisher Token 簽發 |
| `github.com/livekit/server-sdk-go/v2` | Go | `apps/edge-publisher-go/` | Go 端 Room 連線 + Track 發布 |
| `livekit/livekit-server` | v1.7.2 | `docker-compose.dev.yml` | SFU 伺服器容器 |

**LiveKit 的 Webhook 缺口：**

目前 Backend 不接收 LiveKit 的任何事件（如 ParticipantDisconnect、RoomEnded），這導致以下限制：

| 問題 | 影響 |
|------|------|
| 無法得知 Operator 何時斷線（非正常離開） | ControlLease 需等到 30min TTL 到期才釋放，無法即時回收 |
| 無法得知 Edge Agent 意外離線 | 無法觸發後端安全邏輯或發送通知 |
| 無法追蹤 Room 生命週期 | 無 Room 使用統計或空閒清理機制 |

**Room 自動建立**：LiveKit 預設在第一次有人 join 時自動建立 Room（不需事先 createRoom），因此目前無需 Backend 管理 Room。
...
| # | 問題 | 說明 |
|---|------|------|
| 1 | NVENC 版本與 GStreamer plugin 不匹配 | 非 Jetson 或 Ubuntu 版本不對應時，`nvv4l2h264enc` 無法載入，需降級為純 CPU `x264enc` 編碼 |
...
| **P2 — 中期規劃** | | |
| P2 | NVENC 版本不匹配時需 CPU fallback（9.6） | 相容性 | 非 Jetson 平台無法硬體編碼 |
